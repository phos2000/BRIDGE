...
## Check N
ZZ: I just used the entire follow up dataset. Need to ask Lizet about code to define the denominator cohort (there might be some subtleness so better to align with their regular reports on redcap).
...
```
*Lizet suggested we check multiple critical variables (weight, drug, dose, diagnosis?, seizure classification, rate of seizure) that should have a value to convince us that the visit actually took place. Also check the text variable missed_fu to see if there is clear reason that suggests the visit was missed.  

## TO DO: locate variables that suggest missed visit
This may take some time digging into the codebook to locate relevant variables. Please be aware that some questions may have a preset to trigger it (eg. some questions depend on which medication the child is on). Also check BRIDGE_DatIn_rename.R as some variables get assigned a different name.**

```{r miss, eval=FALSE, message=FALSE, warning=FALSE} 

#ZZ: I mute this session currently (eval=FALSE), need to first figure out the list of variables to tell whether it is a missed visit

varl <- c("weight_fu_v2",
          "confirmseize","seizeclass")
# "expseiz","treatment")

tb2x <- fu  %>%
  filter(redcap_repeat_instrument=="follow_up_care_form") %>% 
  mutate(incomplete=
           ifelse(follow_up_care_form_complete<2 |
                    is.na(seizeclass),1,0))
...
tb2x %>%
  ...
  kable(caption = "Records flagged as FU care form not complete or missing seizure classification") %>% 
  kable_styling()
...
#other NAs are not counted anyway



```

## Not sure about missed_fu variable
**ZZ: My understanding is that the team is working on analyzing this text field hopefully to further develop some more useful indicators regarding reason for missing a follow up visit. Currently we should not rely on this field, but can still take a look to see if it explains any data issue.**


```{r check2, eval=TRUE, message=FALSE, warning=FALSE} 
#ZZ: I mute this session currently (eval=FALSE), need to first figure out the list of variables to tell whether it is a missed visit and check missed_fu to see if it is clearly a missed visit as a sanity check.
...
This variable is confusing. Records with a missing fu value can still contain valid scores. Currently this variable is not used in exclusion criteria.  
...
A small number of patients missed the first visit. Any reason?  


## Calculating EQ5D VAS scores (including zero score)
Similar to previous work Neil did to summarize and compare VAS scores across arms.  
**ZZ: This part needs to expand to include sensitivity analysis once we have figured out data exclusion strategies (missed visit, problematic follow up dates, etc).**
...
**Question: if one visit is out of range, do we assess subsequent visits independently?**
eg: visit 1 delayed 1 month, to assess whether visit 2 was on target, should we just calculate days since the actual visit 1?  


## Missing VAS score (Updated on Aug 13)

The table below describes missing (NA) EQ5D VAS scores in all follow-up care form records. Missing score affects less than 1% of all records, 3.1% and 4.6% of individuals on EUC and TSC arm respectively. 4 individuals on TSC arm do not have any valid score (they only contribute one record each).  
...
## Baseline issue?    
**ZZ: VAS was not required for everyone at baseline, only as part of QOLIE-31 and QOLIE-48. Need to decide whether to add baseline values for children between age of 11 and 17.**

...
In the past four weeks, how often have physical or emotional problems caused you to:
Do fewer things than you would have liked to do?
1	Very often  
2	Often  
3	Sometimes  
4	Not often  
...
**ZZ: We decided to ignore the two missing questions and treat the slightly different question 47 same as the original one in scoring.**  


```{r q48}
xw <- readxl::read_xlsx("../qolie/qolie48_xwalk.xlsx",sheet = 2)
...
vv <- data.frame(vn2=names(q48),x=1:length(names(q48)),stringsAsFactors = F) 
#%>% inner_join(xw) 

#rename survey question to align with original questionnaire
for(i in 1:nrow(vv)) {
  names(q48)[which(names(q48)==vv$vn2[i])] <- paste0("q",vv$questionnaire[i])
}
...
score_q48 <- function(df) {
  #epilepsy impact
  ei <- list(
    lq = paste0("q",c(25:27,29:34,36,47,48)),
    lv = paste0("ei",1:12)
  )

...
vv = data.frame(vn_redcap=names(q48c),x=1:length(names(q48c)),stringsAsFactors = F) %>%
...
q48_fu %>% filter(redcap_repeat_instance == 1) %>% left_join(basedate) %>% mutate(datediff = difftime(dateqolie48, datebaseline, units= "days")) %>% count(ev0,datediff) %>% group_by(ev0) %>% mutate(n_sum = sum(n)) %>% kable()

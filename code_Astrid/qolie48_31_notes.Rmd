...
allqolie = allqolie %>% count(record_id, sort = TRUE) %>% rename(qolietimes = n) %>% left_join(allqolie) %>% 
  select(record_id, study_arm, qolietimes, ev0, datebaseline, dateqolie, instance, type, score, miss_item, miss_domain)

allqolie %>%
  arrange(desc(qolietimes), dateqolie, desc(miss_domain), desc(miss_item)) %>%

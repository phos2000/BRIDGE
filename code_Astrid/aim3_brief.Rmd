...
$$QOLIE = 48.2602 + 0.2808 * TSC + 2.3254 * order1 + 4.0736 * order3 - 1.0734*TSC*order1 + 2.4071 * TSC*order3$$
...
$$QOLIE = 34.4684 + 0.2764 * TSC + 2.1276 * order1 + 2.3020 * order3 - 1.0592*TSC*order1 + 2.4055 * TSC*order3 + 0.9673 * age$$
...
$$VAS = 48.564 + 3.804*TSC + 5.090*Visit_1 + 7.981*Visit_2 + 12.403 * Visit_3 + 15.246*Visit_4 + 19.874 * Visit_5 \\
 + 22.715*Visit_6 + 26.013 * Visit_7 + 29.775 * Visit_8 + 30.112 * Visit_9 + 31.555 * Visit_{10} \\
 + 2.287 * TSC * Visit_1 + 5.476 * TSC * Visit_2 + 4.390 * TSC * Visit_3 + 5.996 * TSC * Visit_4 + 4.221 * TSC * Visit_5 \\
 + 5.384 * TSC * Visit_6 + 5.252 * TSC * Visit_7 + 5.656 * TSC * Visit_8 + 6.045 * TSC * Visit_9 + 6.560 * TSC * Visit_{10} $$

```{r lm}
summary(lm(qolie ~ factor(study_arm)*factor(redcap_repeat_instance), fcf_c))

fcf_i = fcf_c %>% filter(!is.na(qolie)) %>% select(record_id, study_arm, visit = redcap_repeat_instance, qolie, age = agefu, gender, city) %>% mutate(gender = ifelse(gender == 1, "Female", "Male")) %>% bind_rows(bs %>% filter(qolie_48_1117_v0_complete == "2") %>% select(record_id = unique_id, study_arm, qolie = vas_qolie_48, age, gender, city) %>% mutate(visit = 0))
...
```{r VAS-QOLIE}
qv = allq %>% filter(order == 0) %>% select(record_id) %>% inner_join(allq %>% filter(order == 3), by = "record_id") %>% select(record_id) %>% left_join(allq) %>% filter(order != 2) %>% count(record_id) %>% filter(n < 3) %>% select(-n) %>% left_join(fcf_c %>% filter(redcap_repeat_instance == 3) %>% mutate(order = 1) %>% select(record_id, study_arm, dateqolie = datefu, qolie, order, age = agefu), by = "record_id")  %>% filter(!is.na(qolie)) %>% bind_rows(allq %>% filter(order == 0) %>% select(record_id) %>% inner_join(allq %>% filter(order == 3), by = "record_id") %>% select(record_id) %>% left_join(allq) %>% filter(order != 2) %>% rename(qolie = score))
...
```{r vas-0thqolie}
new0 = allq %>% filter(order == 3) %>% anti_join(allq %>% filter(order == 0) %>% select(record_id)) %>% select(record_id) %>% left_join(fcf_c %>% filter(redcap_repeat_instance == 1) %>% mutate(order = 0) %>% select(record_id, study_arm, dateqolie = datefu, qolie, order, age = agefu), by = "record_id") %>% filter(!is.na(qolie))

qv1 = new0 %>% select(record_id) %>% left_join(allq) %>% filter(order != 2) %>% rename(qolie = score) %>% bind_rows(new0) %>% bind_rows(new0 %>% select(record_id) %>% left_join(fcf_c %>% filter(redcap_repeat_instance == 3) %>% mutate(order = 1) %>% select(record_id, study_arm, dateqolie = datefu, qolie, order, age = agefu), by = "record_id")) %>% bind_rows(qv) %>% filter(!is.na(qolie))
...
onsetAge %>% 
  ...
  summarize(meanOnsetAge = mean(onset), sdOnsetAge = sd(onset))
...
```{r seizure-times}
#melt
preInterval = cbind(fudw['record_id'], stack(fudw[c("d01","d12", "d23", "d34", "d45", "d56", "d67", "d78", "d89", "d90")])) %>% mutate(instance = ifelse(ind == "d90", 10, as.numeric(ind[-1]))) %>% select(record_id, instance, interval = values)

seizure_tsc = fcf_c %>% filter(!is.na(expseiz) & study_arm == "TSC") %>% mutate(seiz_times = ifelse(is.na(how_many_seiz_v2), 0, how_many_seiz_v2)) %>% left_join(preInterval, by = c("record_id", "redcap_repeat_instance" = "instance")) %>% mutate(seiz_freq = 365.25/12 * seiz_times / interval)
seizure_euc = fcf_c %>% filter(seize_since_last_euc_v2 != 2 & study_arm == "EUC") %>% mutate(seiz_times = ifelse(is.na(number_seiz), 0, number_seiz)) %>% left_join(preInterval, by = c("record_id", "redcap_repeat_instance" = "instance")) %>% mutate(seiz_freq = 365.25/12 * seiz_times / interval)
...
seizure_times %>% mutate(severity = ifelse(seiz_freq >= 1, "severe epilepsy", "seizure-free, treated epilepsy")) %>%
  group_by(study_arm, visit = redcap_repeat_instance) %>% count(severity) %>% mutate(pct = percent(n / sum(n))) %>% filter(severity == "severe epilepsy") %>% ungroup() %>%select(visit,n, pct) %>%
...
DALY = YLL + YLD, while the kind and severity of disease will impact the disability weight for YLD. For severe epilepsy: 0.552 For seizure-free, treated epilepsy: 0.049
...
The first way is to use DALY from literature: 4.1 DALYs per 1000 individuals annually (Wagner, etc, 2015)[<https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0145300>]

as the number use an average disability weight, it is able to infer the DALY for severe and seizure free conditions.

As our 2 arms have different percents of serverity, we can calculate the DALYs for each group.

The second way serves for the cost-effectiveness. Incremental effectiveness = DALY_TSC - DALY_EUC = YLL_TSC + YLD_TSC - YLL_EUC - YLD_EUC
...
If compare them annually per 1000 individuals, incremental effectiveness using DALY weight for each year for the same population size = (4% \* 0.552 + 96% *0.049) - (9%* 0.552 + 91% \* 0.049)

...
beta_parameters <- function(mode, lower, upper, alpha=2)
{
  if(any(mode < lower)) stop("mode must be greater than lower bound")
  if(any(mode > upper)) stop("mode must be lower than upper bound")
  if(any(alpha <= 1))   stop("alpha must be greater than 1")
  
  mode  <- (mode - lower)/(upper - lower)
  
  c(alpha=alpha, beta=2 - alpha +  (alpha - 1) / mode)
}

rtrans_beta <- function(n, mode, lower, upper, alpha=2)
{
  bp <- beta_parameters(mode, lower, upper, alpha)
  
  rbeta(n, bp[1], bp[2] )*(upper - lower) + lower
}

randomBeta = function(mode = 1, lower, upper, n){
  if (upper == 1){
    return(rtrans_beta(n, mode, lower, upper))
  }
...
randomDraw = function(param, params, n){
  ...
  if (length(params_to_use) < 3) {return(rep(params[[param]], n))}
  
  if (params_to_use[[paste0(param, "_dist")]] == "beta") {
    if (!is.null(params_to_use[[paste0(param, "_alpha")]]) & !is.null(params_to_use[[paste0(param, "_beta")]])) {
      return(rbeta(n, params_to_use[[paste0(param, "_alpha")]], params_to_use[[paste0(param, "_beta")]]))
      } else {
      return(randomBeta(mode = params_to_use[[param]], 
                        lower = params_to_use[[paste0(param, "_low")]], 
                        upper = params_to_use[[paste0(param, "_upp")]], 
                        n = n))}

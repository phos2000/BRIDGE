...
| Missed_fu | n_records | n_participants | n
...
So I use the important indices below to distinguish whether they are actually missing or not.
...
cat("Show the inclusive conditions after deleting missing cases\n\n")

fcf2 %>%
  filter(missed == 0) %>%
  count(incomplete, missed_fu,sort = TRUE) %>%
  left_join(fcf2 %>% count(missed_fu), by = c("missed_fu"), suffix = c("_inclusive", "_in_and_ex")) %>%
...
summary(lm(qolie ~ factor(study_arm) + factor(redcap_repeat_instance) + factor(study_arm):factor(redcap_repeat_instance), fcf_c))
...
fudw <- fud %>% select(-datefu) %>% rename(age=agefu) %>% 
  ...
  mutate(d01=as.integer(date_1-date_0),
         ...
         d109=as.integer(date_10-date_9))

save(fcf_c, fudw,file="fcf_dates_wide.rda")

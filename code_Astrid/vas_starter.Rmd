...
*we more focus on the follow-up-care form*  
...
0. follow_up_care_form_complete  
1. diagnosis_of_seizures_v2 = "confirmseize"  
2. weight_fu_v2
...
tb2x %>% filter(redcap_repeat_instrument=="follow_up_care_form", qolie == 50) %>%
  ...
  kable(caption = "is Qolie of 50 a meaningful index for missing data?") %>%
  kable_styling()
...
Which index is the best index? using the missed_fu as a reference
...
# only those missed_fu == NA and have the important values(qolie, weight, seizure_dx, seizure_class) are accepted as existing cases
...
tb2y %>%
  ...
  summarise(n_record=n(),incomplete_num=sum(missed)) %>% ungroup() %>% 
  mutate(pct=percent(incomplete_num/n_record)) %>% 
  kable(caption = "More Strict: Missing Rate in 2 arms") %>% 
  kable_styling()
...
tb2y %>%
  ...
  summarise(n_record=n(),incomplete_num=sum(missed)) %>% ungroup() %>% 
  mutate(pct=percent(incomplete_num/n_record)) %>% 
  kable(caption = "More Strict: Missing rate among all") %>% 
  kable_styling()
```

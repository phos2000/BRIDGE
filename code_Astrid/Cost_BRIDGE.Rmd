...
```{r sa_params}
params_for_sa = c(ls()[grepl("^c[A-Z]", ls())], ls()[grepl("^n[A-Z]", ls())])
params_CI = list()

for (param in params_for_sa) {
  if (startsWith(param, "c")) {
    # cost
    params_CI[[paste0(param, "_dist")]] = "gamma"
  } else {
    params_CI[[paste0(param, "_dist")]] = "normal"
  }
  params_CI[[param]] = eval(parse(text = param))
  params_CI[[paste0(param, "_low")]] = eval(parse(text = param)) * 0.5
  params_CI[[paste0(param, "_upp")]] = eval(parse(text = param)) * 1.5
}
...
```{r cost_sum}
cTSC_sum = function(){
  cVisit_CHW_2023*exchange_Naira_to_USD*nVisit_CHW_TSC +
...
cEUC_sum = function(){
  cVisit_CHW_2023*exchange_Naira_to_USD*nVisit_CHW_EUC +
...
```{r cost_SA}
source("random_draws_for_BRIDGE_PSAs.R")

n_PSA = 10000
n_cores = detectCores() - 1

params_psa = sapply(params_for_sa, function(param) {
  set.seed(101)
  print(param)
  randomDraw(param, params_CI, n_PSA)})

costs_PSA <- mclapply(1:n_PSA, mc.cores = n_cores, FUN = function(x){
  sapply(params_for_sa, function(param){ assign(param, params_psa[x, param], envir = .GlobalEnv)})
  incremental_cost_per = cTSC_sum() / nrow(severity_DALY[severity_DALY[["study_arm"]] == "TSC",]) - 
  cEUC_sum() / nrow(severity_DALY[severity_DALY[["study_arm"]] == "EUC",])
  return(incremental_cost_per)
})

incremental_cost_per
hist(unlist(costs_PSA))
```

## Bootstrap for Distribution of DALY-related seizure freq

```{r}
severity_TSC = subset(severity_DALY, study_arm == "TSC")
severity_EUC = subset(severity_DALY, study_arm == "EUC")

bootstrap_psa <- unlist(mclapply(1:n_PSA, mc.cores = n_cores, FUN = function(x){
  sample_TSC = severity_TSC[sample(1:nrow(severity_TSC), size = nrow(severity_TSC), replace = TRUE), ]
  sample_EUC = severity_EUC[sample(1:nrow(severity_EUC), size = nrow(severity_EUC), replace = TRUE), ]
  return(mean(sample_TSC[,"DALY_2yr"]) - mean(sample_EUC[,"DALY_2yr"]))
  }))
```
...
```{r}
CEA_PSA = unlist(map2(unlist(costs_PSA), bootstrap_psa,function(x, y){x/-y}))
